...
596 MiB, LiveKit 약 40 MiB다. 측정 당시 CPU 부하는 낮았지만 실제 팬미팅 진행 중의
부하는 아직 측정하지 않았다.
...
메모리와 디스크는 PoC에 충분하다. 운영 배치의 핵심 판단 항목은 CPU이며, 실제 통화와
녹화가 동시에 진행되는 5단계 부하 측정 결과가 필요하다. 운영에서 여러 녹화를 동시에
수행해야 한다면 Egress worker를 별도 서버로 분리하는 것을 우선 검토한다.
