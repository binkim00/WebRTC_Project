...
type EditorTarget =
  | { mode: 'create' }
  | { mode: 'edit'; noticeId: number; title: string; content: string }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (editor.mode === 'create') {
        await createServiceNotice({ title, content }, authToken)
        setNotice(t('adminServiceNoticesPage.t24'))
        // 새 공지는 최신순 목록의 첫 페이지에 들어가므로 그쪽을 보여 준다.
        setPage(0)
      } else {
        // PATCH이므로 바뀐 값만 보낸다. 둘 다 그대로면 요청하지 않는다.
        const changed = {
      ...
      <Dialog
        footer={
          <>
            ...
            <Button form="admin-notice-form" loading={saving} type="submit">
