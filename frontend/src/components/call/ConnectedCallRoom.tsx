...
export function ConnectedCallRoom({
  ...
  /**
   * 같이 찍기 카운트다운의 현재 숫자다. 세는 중이 아니면 undefined다.
   *
   * 셔터를 누르면 곧바로 찍히지 않고 3·2·1을 센다. 상대에게도 같은 카운트를 띄워, 남는 사진이
   * "몰래 찍힌 화면"이 아니라 **양쪽이 포즈를 잡고 함께 찍은 사진**이 되게 하려는 것이다.
   */
  const [shutterCountdown, setShutterCountdown] = useState<number>()
  /**
   * 카운트 중 셔터 연타를 막는 잠금이다.
   *
  ...
  useEffect(() => {
    ...
    // 이전 팬이 누른 셔터의 카운트다운이 새 통화 화면에 남지 않게 한다.
  ...
  return (
    ...
    <div className="-mx-3 -my-5 min-h-[calc(100dvh-var(--service-header-height))] bg-[var(--color-surface-dark)] px-4 pb-8 pt-[22px] sm:-mx-6 sm:px-6 lg:-mx-10 lg:-my-6 lg:px-10">
      ...
      <CallStage
        ...
        // 카운트 중에는 셔터를 잠가 연타로 카운트다운이 겹치지 않게 한다.
        captureDisabled={!canCapture || capturing || shutterCountdown !== undefined}
        captureLabel={
          shutterCountdown !== undefined
            ? t('connectedCallRoom.captureCountdown', { p0: shutterCountdown })
            : capturing
