...
/**
 * SMTP 발송 인프라가 없는 환경에서 재설정 링크를 로그로만 남기는 개발용 구현이다.
 *
 * <p>이 구현이 그대로 운영에 올라가도 링크가 새어 나가지 않도록, 링크 출력은
 * {@link PasswordResetPolicy}가 개발 프로파일에서만 허용한 경우에 한정한다.
 */
@Component
...
public class LoggingPasswordResetMailSender implements PasswordResetMailSender {
    ...
    /**
     * 토큰 노출 허용 여부를 판단할 재설정 정책을 주입받는다.
     *
     * @param policy 비밀번호 재설정 정책
     */
    public LoggingPasswordResetMailSender(PasswordResetPolicy policy) {
        this.policy = policy;
    }

    /**
     * 재설정 링크를 발송하는 대신 로그로 남긴다.
     *
    ...
    public void send(User user, String resetLink, LocalDateTime expiresAt) {
        if (policy.exposeToken()) {
            log.info("[개발용] 비밀번호 재설정 링크 userId={} expiresAt={} link={}",
                    user.getId(), expiresAt, resetLink);
            return;
        }
        // 링크와 이메일 주소를 남기지 않고 발송 시도 사실만 기록한다.
        log.warn("비밀번호 재설정 메일을 발송할 SMTP 구현이 없습니다. userId={} expiresAt={}",
