...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    /**
     * 특정 팬미팅의 커뮤니티 게시글 목록을 작성자·팬미팅과 함께 한 번에 조회한다.
     *
     * <p>작성자와 팬미팅을 fetch join해 목록 매핑 중 건별 추가 조회가 발생하지 않도록 한다.
     * 검색어를 사용하지 않을 때는 모든 행과 일치하는 {@code %} 패턴을 넘겨
     * 파라미터가 null인 비교식을 만들지 않는다.
     *
     * @param meetingId 대상 팬미팅 식별자
     ...
     * @param keyword 소문자로 변환한 제목·본문 LIKE 패턴이며 전체 조회는 {@code %}
     * @param pageable 정렬과 페이지 정보
     * @return 삭제·숨김되지 않은 해당 팬미팅의 커뮤니티 게시글 페이지
     */
    @Query(value = """
            select p from Post p
            join fetch p.author
            join fetch p.meeting m
            where p.type = com.ssafy.backend.post.domain.PostType.COMMUNITY
              and m.id = :meetingId
              and p.status = :status
              and p.deletedAt is null
              and (lower(p.title) like :keyword or lower(p.content) like :keyword)
            ...
            where p.type = com.ssafy.backend.post.domain.PostType.COMMUNITY
              ...
              and (lower(p.title) like :keyword or lower(p.content) like :keyword)
            """)
    Page<Post> findVisibleCommunityPosts(@Param("meetingId") Long meetingId,
                                         @Param("status") PostStatus status,
                                         @Param("keyword") String keyword,
                                         Pageable pageable);
