...
/**
 * 녹화 파일을 서버 디스크에 저장하고 읽고 지운다.
 *
 * <p>S3나 MinIO를 쓰지 않고 설정된 최상위 경로 아래에만 파일을 둔다. 저장 경로에는 원본 파일명을
 * 쓰지 않고 UUID 기반 키를 사용하며, 모든 경로 계산은 최상위 경로를 벗어나지 않는지 검사한다.
 */
@Component
public class RecordingFileStorage {

    private static final Logger log = LoggerFactory.getLogger(RecordingFileStorage.class);
     ...
     */
    public RecordingFileStorage(RecordingStorageProperties properties) {
        ...
        try {
            ...
            throw new IllegalStateException(
                    "녹화 저장 디렉터리를 준비하지 못했습니다: " + this.storageRoot, exception);
        }
        log.info("녹화 저장 경로를 사용합니다. path={}", this.storageRoot);
    }
    ...
    public long store(String storageKey, InputStream source, long maxSizeBytes) {
        ...
        try {
            ...
            deleteQuietly(temp);
            log.warn("녹화 파일 저장에 실패했습니다. storageKey={}", storageKey, exception);
     ...
     * 역슬래시가 들어 있으면 그 자체가 조작 신호로 보고 거부한다. 상위 참조 개수에 따라 결과가
     * 우연히 최상위 경로 안으로 떨어지는 경우까지 막기 위해, 경계 검사와 별도로 검사한다.
     * 역슬래시는 운영체제마다 구분자 취급이 달라 판정이 흔들리므로 함께 거부한다.
     *
     * @param storageKey 저장 키
    ...
    private BusinessException rejectedKey(String storageKey) {
        log.warn("허용하지 않는 녹화 저장 키를 거부했습니다. storageKey={}", storageKey);
        return new BusinessException(ErrorCode.RECORDING_NOT_FOUND);
    }
     ...
     */
    private long copyWithLimit(InputStream source, Path temp, long maxSizeBytes) throws IOException {
        long written = 0L;
